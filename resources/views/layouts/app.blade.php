...
<body class="flex flex-col  min-h-screen {{ auth()->user() ? 'bg-gray-50' : 'bg-white' }}">
<header class="p-5 border-b  shadow bg-white">
    <div class="container mx-auto flex flex-col md:flex-row items-center gap-5 justify-between">
        <div class="flex flex-col md:flex-row gap-10 items-center">
            <a href="{{ auth()->user() ? route('home') : route('index') }}"
        ...
        @auth
            ...
            <nav class="flex flex-col md:flex-row gap-5 py-5 lg:py-0 items-center">
                <a href="{{route('publicaciones.create')}}" class="flex items-center gap-2 bg-white border p-2 text-gray-600 rounded
                ...
                <a href="{{route('publicaciones.index', auth()->user()->nombre_usuario)}}"
            ...
            @if(Route::is('index'))
                <nav class="flex gap-3">
                    @foreach($redes as $red)
                        <a href="{{ env('URL_PORTAFOLIO_SNAISDEV') ?? 'https://github.com/snaispro77' }}"
                           class="w-10 h-10 flex items-center justify-center rounded-lg bg-white shadow-md shadow-gray-200 group transition-all duration-300">
                            <img src="{{ asset("img/redes/{$red['logo_url']}")}}" alt="red">
                        </a>
                    @endforeach
                </nav>
            @else
                <nav class="flex gap-x-5 items-center">
                    ...
                    <a class="font-bold uppercase text-gray-600 text-sm hover:text-gray-800"
                       href="{{ route('registro.index') }}">Crear Cuenta
                    </a>
                </nav>
            @endif
        @endguest
...
<footer class="mt-10 py-6  font-bold uppercase text-center
        {{ auth()->user() ? 'text-gray-500': 'text-gray-300 bg-cyan-950'}}
    ">
    NetDev - Todos los derechos reservados por @SnaisDev {{ now()->year }}
</footer>
@livewireScripts
</body>
</html>
